...
def initPlot():
    ...
    ax.set_xlim3d([-axes_limit, axes_limit])
    ax.set_xlabel('X (m)')
    ax.set_ylim3d([-axes_limit, axes_limit])
    ax.set_ylabel('Y (m)')
    ax.set_zlim3d([-axes_limit, axes_limit])
    ax.set_zlabel('Z (m)')
...
def plotAxes(show_path):
    ...
    def getJointAxes(theta):
        ...
        for i in range(len(fwd_kin)):
            ...
            tmp_x, tmp_y, tmp_z, one = np.matmul(
                fwd_kin_chain, np.transpose(origin))
...
def simplePath(start, stop, interval, show_path):
    joints = []
    if show_path:
        interval = int(np.floor(2*interval))
    for i in range(len(robot.theta)):
...
global axes_limit
axes_limit = sum(np.abs(np.array(robot.a)))
initPlot()
